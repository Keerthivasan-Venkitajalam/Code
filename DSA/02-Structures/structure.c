...
date getDate() {
	date d;
	//TODO
	char* dat=malloc(sizeof(char)*11);
	scanf("%[^\n]%*c",dat);
	char* c=malloc(sizeof(char)*3);
	sscanf(dat,"%d-%3s-%d",&d.day,c,&d.year);
	if(strcmp("jan",c)==0){
		d.month=1;
	}
	else if(strcmp("feb",c)==0){
		d.month=2;
	}
	else if(strcmp("mar",c)==0){
		d.month=3;
	}
	else if(strcmp("apr",c)==0){
		d.month=4;
	}
	else if(strcmp("may",c)==0){
		d.month=5;
	}
	else if(strcmp("jun",c)==0){
		d.month=6;
	}
	else if(strcmp("jul",c)==0){
		d.month=7;
	}
	else if(strcmp("aug",c)==0){
		d.month=8;
	}
	else if(strcmp("sep",c)==0){
		d.month=9;
	}
	else if(strcmp("oct",c)==0){
		d.month=10;
	}
	else if(strcmp("nov",c)==0){
		d.month=11;
	}
...
int isValidDate(date d) {
	...
	if((d.year)>=1900&&(d.year)<=2050){
		...
		else if((d.month)==2&&(d.year)%4==0&&(d.year)%100!=0||(d.year)%400==0&&(d.day)<=29&&(d.day)>=1){
			return 1;
		}
